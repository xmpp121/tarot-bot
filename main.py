...
# ---------- КОНФИГ ----------
BOT_TOKEN = "8050194596:AAGoCprj1aDw4UwzhBQ2Ma8Uc-O4TBFhQaU"  # ваш токен
CHANNEL_CHAT_ID = -1001234567890  # <-- ВСТАВЬТЕ chat_id вашего КАНАЛА (формат -100xxxxxxxxxx)
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3) Пытаемся удалить контакт-сообщение клиента (чтобы он его не видел в истории)
    try:
        await msg.delete()
        log.info("User contact message deleted")
    except Exception as e:
        # В частных чатах обычно удаляется; в группах требуется право админа на удаление
        log.warning("Не удалось удалить контакт-сообщение: %s", e)

    # 4) Отправляем аккуратное подтверждение
    try:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
...
def main():
    if not BOT_TOKEN:
        raise RuntimeError("Укажите BOT_TOKEN в коде")
    ...
    log.info("Starting bot (long polling). Если webhook был — игнорируется.")
    app.run_polling(
